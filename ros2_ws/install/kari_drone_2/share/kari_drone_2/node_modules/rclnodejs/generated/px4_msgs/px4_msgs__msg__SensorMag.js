...
const SensorMagRefStruct = StructType({
...
const SensorMagRefArray = ArrayType(SensorMagRefStruct);
const SensorMagRefStructArray = StructType({
  data: SensorMagRefArray,
  size: ref.types.size_t,
  capacity: ref.types.size_t
});
// Define the wrapper class.
class SensorMagWrapper {
  constructor(other, willCheckConsistency = false) {
    this._wrapperFields = {};
    this._willCheckConsistency = willCheckConsistency;
    if (typeof other === 'object' && other._refObject) {
      this._refObject = new SensorMagRefStruct(other._refObject.toObject());
  ...
  _initMembers() {
    this._refObject = new SensorMagRefStruct();
  ...
  static createFromRefObject(refObject) {
    let self = new SensorMagWrapper();
    self.copyRefObject(refObject);
    return self;
  }
  static createArray() {
    return new SensorMagArrayWrapper;
  }
  static get ArrayType() {
    return SensorMagArrayWrapper;
  }
  static get refObjectArrayType() {
    return SensorMagRefStructArray
  }
  static get refObjectType() {
    return SensorMagRefStruct;
  }
  toRawROS() {
    this.freeze(true);
  ...
  freeze(own = false, checkConsistency = false) {
      if (checkConsistency && !this._timestampIntialized) {
        throw new TypeError('Invalid argument: timestamp in SensorMag');
      }
      if (checkConsistency && !this._timestamp_sampleIntialized) {
        throw new TypeError('Invalid argument: timestamp_sample in SensorMag');
      }
      if (checkConsistency && !this._device_idIntialized) {
        throw new TypeError('Invalid argument: device_id in SensorMag');
      }
      if (checkConsistency && !this._xIntialized) {
        throw new TypeError('Invalid argument: x in SensorMag');
      }
      if (checkConsistency && !this._yIntialized) {
        throw new TypeError('Invalid argument: y in SensorMag');
      }
      if (checkConsistency && !this._zIntialized) {
        throw new TypeError('Invalid argument: z in SensorMag');
      }
      if (checkConsistency && !this._temperatureIntialized) {
        throw new TypeError('Invalid argument: temperature in SensorMag');
      }
      if (checkConsistency && !this._error_countIntialized) {
        throw new TypeError('Invalid argument: error_count in SensorMag');
      }
  }
  serialize() {
  ...
  static destoryRawROS(msg) {
    SensorMagWrapper.freeStruct(msg.refObject);
  }
  static type() {
    return {pkgName: 'px4_msgs', subFolder: 'msg', interfaceName: 'SensorMag'};
  }
  static isPrimitive() {
    return false;
  }
  ...
  copyRefObject(refObject) {
    this._refObject = new SensorMagRefStruct(refObject.toObject());
    this._timestampIntialized = true;
    this._timestamp_sampleIntialized = true;
    this._device_idIntialized = true;
    this._xIntialized = true;
...
class SensorMagArrayWrapper {
  ...
  fill(values) {
    ...
    values.forEach((value, index) => {
      if (value instanceof SensorMagWrapper) {
        this._wrappers[index].copy(value);
      } else {
        this._wrappers[index] = new SensorMagWrapper(value);
      }
    });
  }
  ...
  set size(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to SensorMagArrayWrapper.size setter');
      return;
    }
  ...
  set capacity(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to SensorMagArrayWrapper.capacity setter');
    }
    return this._resize(value);
  }
  get refObject() {
  ...
  _resize(size) {
    ...
    this._refArray = new SensorMagRefArray(size);
    this._refObject = new SensorMagRefStructArray();
    this._refObject.size = size;
    this._refObject.capacity = size;
    this._wrappers = new Array();
    for (let i = 0; i < size; i++) {
      this._wrappers.push(new SensorMagWrapper());
    }
  }
  ...
  copy(other) {
    if (! (other instanceof SensorMagArrayWrapper)) {
      throw new TypeError('Invalid argument: should provide "SensorMagArrayWrapper".');
    }
  ...
  static freeArray(refObject) {
    ...
    for (let index = 0; index < refObject.size; index++) {
      SensorMagWrapper.freeStruct(refObjectArray[index]);
    }
  }
...
Object.defineProperty(SensorMagWrapper, "ORB_QUEUE_LENGTH", {value: 4, writable: false, enumerable: true, configurable: true});
module.exports = SensorMagWrapper;
/*
